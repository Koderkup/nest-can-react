...
export async function generateFlightEntries({ config, pages }) {
  ...
  await writePageRefs(config.rootDir, pages);
...
async function writePageRefs(rootDir, pages) {
  const sourceRoot = readSourceRoot(rootDir);
  const target = join(rootDir, sourceRoot, 'react-pages.ts');
  const exportsByName = new Map();

  for (const page of pages) {
    const exportName = pageRefExportName(page.name);
    const existing = exportsByName.get(exportName);

    if (existing) {
      throw new Error(
        `Page refs ${existing} and ${page.relativeFile} both export ${exportName}. Rename one of the page files.`,
      );
    }

    exportsByName.set(exportName, page.relativeFile ?? page.file);
  }
  ...
  if (exportsByName.size === 0) {
    lines.push('export {};', '');
  } else {
    lines.push(`import { createPageRef } from 'nest-can-react';`, '');

    const sorted = [...exportsByName.entries()].sort(([a], [b]) =>
      a.localeCompare(b),
    );

    for (const [exportName, relativeFile] of sorted) {
      const page = pages.find(
        (entry) => (entry.relativeFile ?? entry.file) === relativeFile,
      );
      lines.push(`/** ${toPosixPath(relativeFile)} */`);
      lines.push(
        `export const ${exportName} = createPageRef(${JSON.stringify(page.name)});`,
      );
    }
  ...
  await writeFileIfChanged(target, lines.join('\n'));
}

function readSourceRoot(rootDir) {
  const nestCliPath = join(rootDir, 'nest-cli.json');
  ...
  try {
    const json = JSON.parse(readFileSync(nestCliPath, 'utf8'));
    if (typeof json.sourceRoot === 'string' && json.sourceRoot.length > 0) {
      return json.sourceRoot.replace(/\\/g, '/').replace(/\/$/, '');
    }
  } catch {
    // fall through
  }
...
function pageRefExportName(pageName) {
  const pascal = String(pageName)
    .split(/[^a-zA-Z0-9]+/)
    .filter(Boolean)
  ...
  if (!pascal) {
    throw new Error(`Cannot name a page ref for "${pageName}".`);
  }

  const named = pascal.endsWith('Page') ? pascal : `${pascal}Page`;

  if (!/^[A-Za-z_$]/.test(named)) {
    return `Page${named}`;
  }
